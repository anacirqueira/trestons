/*
 -- Jogo da advinhação --

 Apresente a mensage ao usuário: 
 "Advinhe o número que estou pensando? Está entre 0 e 10"

 Crie uma lógica para gerar um número aleatório 
 e verificar se o número digitado é o mesmo que foi gerado pelo sistema.

 Enquanto o usuário não advinhar o número, mostrar a mensagem:
 "Erro, tente novamente:"

 Caso o usuário acerte o número, apresentar a mensagem:
 "Parabéns! Você adivinhou o número em x tentativas"

 Substitua o "x" da mensagem, pelo número de tentativas.
*/

let result = prompt("Advinhe o número que estou pensando? Está entre 0 e 10")

const numeroAleatorio = Math.round(Math.random() * 10)

let xTentativas = 1

while(numeroAleatorio != result) {
  result = prompt("Erro, tente novamente:")
  xTentativas++
}

alert(`Parabéns! Você adivinhou o número em ${xTentativas} tentativas`)